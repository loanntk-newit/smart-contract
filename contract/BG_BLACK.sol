...
contract BG_BLACK is ERC721A, Ownable, IERC4906 {
  ...
  function handleMint(
    ...
    startNextPhase();
    require(secondsRemaining(phaseEndTime) > 0, 'Minting for this phase has ended');
    if (_mintPack) {
  ...
  function combine(uint256[] memory tokens) public {
    ...
    for (uint256 i = 1; i <= currentPhase; i++) {
      require(
        !findString(newValue, phaseKey[i]),
  ...
  function matchStr(uint256[] memory _tokens) public {
    require(!isCombinable, 'cannot perform this action when combining is active');
  ...
  function claimReward(uint256[] memory _tokens) public {
    ...
    bool isWin = compare(string(newValue), string(superKey));
  ...
  function getMetadata(
    ...
    if (isKey) {
      ...
      string memory path = string(
        abi.encodePacked('{"trait_type": "Alphabet", "value": "', value, '"}, ')
      );
      if (combined) {
        path = string(abi.encodePacked('{"trait_type": "Combine", "value": "', value, '"}, '));
      }
      json = string(
        abi.encodePacked(
          '{"name": "BG_BLACK #',
          ...
          uint2str(currentPhase),
          '"}, {"trait_type": "Burned", "value": "No"}, {"trait_type": "Color", "value": "',
          baseColorNames[currentPhase - 1],
          '"}], "image": "data:image/svg+xml;base64,',
          Base64.encode(bytes(renderSvg(value, baseColorNames[currentPhase - 1]))),
          '"}'
        )
      );
    }
  ...
  function isCharInString(string memory c, string memory s) internal pure returns (bool) {
  ...
  function setPhaseValues(uint256 _phase, string memory _values) public onlyOwner {
  ...
  function findString(string memory _string, string[] memory array) internal pure returns (bool) {
  ...
  function withdraw(uint256 _amount) external onlyOwner {
    (bool hs, ) = payable(winningWallets).call{ value: (address(this).balance * _amount) / 100 }(
      ''
    );
    require(hs);
